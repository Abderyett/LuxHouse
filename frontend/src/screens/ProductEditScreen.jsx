...
export function ProductEditScreen() {
	...
	useEffect(() => {
		const featuresArr = featuresList.map(
			(el) => el.Features.charAt(0).toUpperCase() + el.Features.slice(1)
		);
	...
	const submitColorsHandler = (event) => {
		...
		if (event.code === "Enter") {
			if (addedColor.length === 6) {
				setColorsList((prevState) => [
					...prevState,
					{ color: `#${addedColor}`, objectID: uuidv4() },
				]);
				setAddedColor("");
			}
		}
		...
		if (event.code === "Enter") {
			if (addedFeature.length > 0) {
				setFeaturesList((prevState) => [
					...prevState,
					{ Features: addedFeature, objectID: uuidv4() },
				]);
	...
	const uploadImage = async (base64EncodedImage) => {
		...
		try {
			const { data } = await axios.post(
				"https://lux-house.onrender.com/api/v1/upload",
				jsonData,
				config
			);
	...
	const uploadMultipleImages = async (imgArr) => {
		...
		try {
			const { data } = await axios.post(
				"https://lux-house.onrender.com/api/v1/upload/multiple",
				payload,
				config
			);
								...
								{({
									...
									>
										...
										<InputWrapper>
											...
											<SubcategoryWrap
												showDropDownSubCategory={showDropDown}
												onClick={selectedSubcategoryHandler}
											>
