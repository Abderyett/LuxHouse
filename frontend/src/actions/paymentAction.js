...
export const proceedPayment = (item) => async (dispatch, getState) => {
	...
	try {
		const { data } = await axios.post(
			"https://lux-house.netlify.app/create-checkout-session",
			item,
			config
		);
...
export const updateStripePayment = (id) => async (dispatch, getState) => {
	...
	try {
		const { data } = await axios.get(
			`https://lux-house.onrender.com/api/v1/orders/${id}/stripe`,
			config
		);
...
export const updatePaypalPayment = (orderId, paymentResult) => async (dispatch, getState) => {
	...
	try {
		const { data } = await axios.put(
			`https://lux-house.onrender.com/api/v1/orders/${orderId}/paypal`,
			paymentResult,
			config
		);
