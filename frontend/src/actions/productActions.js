...
export const listProduct = () => async (dispatch) => {
	try {
		...
		const { data } = await axios.get("https://lux-house.onrender.com/api/v1/products");
		dispatch({
...
export const detailProduct = (id) => async (dispatch) => {
	try {
		...
		const { data } = await axios.get(`https://lux-house.onrender.com/api/v1/products/${id}`);
...
export const removeProduct = (id) => async (dispatch, getState) => {
	try {
		...
		await axios.delete(`https://lux-house.onrender.com/api/v1/products/${id}`, config);
...
export const createProductAC = () => async (dispatch, getState) => {
	try {
		...
		const { data } = await axios.post(`https://lux-house.onrender.com/api/v1/products`, {}, config);
...
export const updateProductAC = (id, product) => async (dispatch, getState) => {
	try {
		...
		const { data } = await axios.put(
			`https://lux-house.onrender.com/api/v1/products/${id}`,
			product,
			config
		);
